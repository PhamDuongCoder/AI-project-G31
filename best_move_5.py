"""best_move without killer move heuristic , move ordering, quiescence search and alpha beta"""
"""This bot only has pure minmax algorithm without any optimizations"""
import chess
import random
...
def get_best_move_minimax(board: chess.Board, depth: int = 3) -> Optional[chess.Move]:
    """Return best move found by minimax (no alpha-beta) from the current board."""
    legal_moves = list(board.legal_moves)
    if not legal_moves:
        return None
...
def minimax(board: chess.Board, depth: int, maximizing: bool, root_color: bool, ply: int = 0) -> float:
    ...
    if maximizing:
        ...
        for move in legal_moves:
            board.push(move)
            # Không truyền alpha, beta. Chiều sâu giảm 1, maximizing đảo ngược
            eval_score = minimax(board, depth - 1, False, root_color, ply=ply+1)
            board.pop()
            
            # Chỉ tìm giá trị lớn nhất (Max)
